...
bool QgsWidgetStateHelper::eventFilter( QObject *object, QEvent *event )
{
  if ( event->type() == QEvent::ToolTip )
  {
    ...
    if ( widget->property( "widgetDisableTooltipHelper" ).toString() )
    {
      QToolTip::showText( widget->property( "widgetDisableTooltipHelper" ).toString() + widget->toolTip() );
    }
  }
...
void QgsWidgetStateHelper::registerWidget( QWidget *widget, const QString &key )
{
  const QString name = widgetSafeName( widget );
  mKeys[name] = key;
  widget->setProperty( "widgetDisableTooltipHelper", QVariant( key ) );
